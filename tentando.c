...
void Save(tipoTabela tabelaA){
    FILE *filePointer;
    int i, cont;

    filePointer = fopen("save.txt", "w");
    ...
    if ( filePointer == NULL ) {
        ...
        for(int i = 0; i < tam_mundo; i++){
            for(int j=0;j< tam_mundo;j++){
                if(tabelaA[i][j] == vida_sim){
                    cont++;
                    printf("oi 2");
                }
            }
        }
        fprintf(filePointer,"Numero de seres: %d\n", cont);
        printf("oi 3");
         for(int i = 0; i < tam_mundo; i++){
            for(int j=0;j< tam_mundo;j++){
                if(tabelaA[i][j] == vida_sim){
                    printf("oi 4");
                    fprintf(filePointer, "P: %d %d\n", &i, &j);
                }
...
void carregarJogo(tipoTabela tabela){
    FILE *filePointer;
    int cont, i, j, k;

    filePointer = fopen("save.txt", "r") ;

    if ( filePointer == NULL ) {
        printf( "nao foi possivel abrir o arquivo" );
    }else{
        fscanf(filePointer,"Numero de seres: %d\n", &cont);
        for(i=0;i<cont;i++){
            fscanf(filePointer, "P: %d %d\n", &k, &j);
            tabela[k][j] = vida_sim;
        }
    }
    fclose(filePointer);
